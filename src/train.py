...
def train_and_evaluate(forecast_feature_bank: pd.DataFrame) -> Tuple[Dict[str, lgb.LGBMRegressor], pd.DataFrame]:
    ...
    predictor_columns = forecast_feature_bank.drop(columns=['hourly_energy_demand'] + [f'target_h{h}' for h in range(1, forecast_horizon_hours + 1)])
    prediction_targets = forecast_feature_bank[[f'target_h{h}' for h in range(1, forecast_horizon_hours + 1)]]
    split_index = int(len(forecast_feature_bank) * 0.8)
    ...
    evaluation_results = []
    lgb_params = {'objective': 'regression', 'n_estimators': 500, 'learning_rate': 0.05, 'num_leaves': 31, 'subsample': 0.8, 'colsample_bytree': 0.8, 'random_state': 42, 'verbosity': -1}
    with mlflow.start_run(run_name='Production_Training'):
        ...
        for h in range(1, forecast_horizon_hours + 1):
            ...
            mlflow.lightgbm.log_model(lgb_model=model, artifact_path=f'model_h{h}', registered_model_name=f'GridCastAI_Model_H{h}')
