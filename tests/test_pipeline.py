...
# Adjust path so pytest can find src and api modules if run from root
import sys
import os
...
def test_clean_data_logic():
    """Unit test for the clean_data transformation."""
    # Create a dummy DataFrame matching the raw expected input
    df = pd.DataFrame({
        "Date": ["16/12/2006", "16/12/2006"],
        "Time": ["17:24:00", "18:24:00"],
...
def test_feature_engineering_logic():
    ...
    # Create dummy hourly data spanning enough hours for lag calculation
    dates = pd.date_range(start="2006-12-16", periods=200, freq="1h")
    df = pd.DataFrame({
        "hourly_energy_demand": [x for x in range(200)]
    }, index=dates)
    
    features = feature_engineering(df)
    
    # Check if lags and targets were created
